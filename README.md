# ðŸš† TrainTrack â€” Mumbai Local Train Tracker

**Real-time departures, delays, and platform numbers for Western, Central and Harbour lines.**

> A fully offline-capable **Progressive Web App** built on zero-framework Vanilla JS. Install it directly from your browser â€” no app store required.
...
## ðŸŒ¿ Architectural Philosophy: "Walk in the Garden"

TrainTrack was built under the **"Walk in the Garden"** methodology â€” a phased, compute-efficient approach to PWA development:

| Phase | Name | Principle |
|-------|------|-----------|
| ðŸŒ± **Seed** | App Shell | Establish the HTML/CSS structure before any logic |
| ðŸ’§ **Water** | Logic & Integration | Pour modular, async JS into data-binding points |
| âœ‚ï¸ **Prune** | Optimise | Remove reflow triggers, batch DOM writes in RAF |
| ðŸŒ¸ **Bloom** | PWA Resilience | Service Worker, offline mode, background sync |

**Core constraint: Extreme Compute Efficiency**
- Zero JS framework â€” pure `TrainTrack.*` namespace
...
```
TrainTrack/
â”œâ”€â”€ index.html          â† App shell (semantic HTML5)
â”œâ”€â”€ css/                â† Amber Dawn design system + screen styles
â”œâ”€â”€ js/                 â† ES modules (`js/app.js` boots `window.TrainTrack`)
â”œâ”€â”€ sw.js               â† Service Worker (4 caching strategies)
â”œâ”€â”€ manifest.json       â† PWA manifest (maskable icons, shortcuts)
â”œâ”€â”€ schedules.json      â† Static seed dataset (Mumbai local timetables)
â”œâ”€â”€ icons/              â† PWA icons (192px, 512px, maskable variants)
â””â”€â”€ mcp/
    â””â”€â”€ cloud-run/      â† Cloud Run MCP server (GCP service management)
```

---

## ðŸ“¦ TrainTrack Module Map (`js/app.js`)
...
### `TrainTrack.Config`
Frozen object of all constants: API endpoints, localStorage keys, IndexedDB config, line colours, and poll intervals. **Single source of truth** â€” change one value, it propagates everywhere.

### `TrainTrack.Store`
Two-tier persistence:
- **localStorage** â€” user preferences (line, from/to stations, clock format, auto-refresh) and favourite routes
- **IndexedDB** â€” `schedules.json` binary cache for instant offline paint on next cold start

### `TrainTrack.API`
Fetch wrapper implementing **Stale-While-Revalidate**:
1. Check in-memory `Map` cache â€” return immediately if fresh
...
### `TrainTrack.App`
Bootstrap sequence:
1. **Instant paint** â€” serve IndexedDB-cached `schedules.json` if available
2. **Background fetch** â€” re-fetch `schedules.json` from the network, update IDB
3. **UI wiring** â€” Amber Dawn home + journey tracker screens (in progress)
4. **Live hydration** â€” per-card RailRadar lookups (best-effort; failures are silent)
...
**Background Sync** (`sync-schedules` tag): If the app goes offline mid-session, the SW queues a `schedules.json` refresh. When the connection restores â€” even if the app is closed â€” the SW fetches the latest schedule silently.
...
## ðŸŽ¨ Design System: "Amber Dawn" (in progress)
...
- Animations use only `transform` + `opacity` â€” zero layout-triggering properties
...
**iOS note:** Mobile Safari supports PWAs, but **Service Worker update semantics and some background capabilities differ** from Chromium. If something looks â€œstuckâ€ after a deploy, close all tabs and re-open the site once (or remove/re-add the Home Screen icon) to pick up the latest `sw.js`.

### Deploy to GitHub Pages
```bash
git push origin main
# Enable Pages in repo Settings â†’ Pages â†’ Deploy from main branch
```
...
**Note:** RailRadar has no guaranteed stable public contract; endpoints and payload shapes may change. TrainTrack treats live hydration as best-effort (timeouts + silent fallback to static schedules).
...
All API calls fail **silently** â€” the UI always renders from the static `schedules.json` first, then hydrates with live data if available. Network errors never crash the app.

To add your own API key:
```js
// js/app.js â†’ TrainTrack.Config
RAILRADAR_BASE: 'https://your-api-proxy.example.com/api',
```

---

## ðŸ—º Data: `schedules.json`
